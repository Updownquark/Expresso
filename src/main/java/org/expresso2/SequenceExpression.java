...
public class SequenceExpression<S extends BranchableStream<?, ?>> extends ExpressionComponent<S> {
	...
	private static class SequencePossibilities<S extends BranchableStream<?, ?>> implements PossibilitySequence<Expression<S>> {
		...
		public Expression<S> getNextPossibility() throws IOException {
			...
			if (!isStarted) {
				isStarted = true;
				PossibilitySequence<? extends Expression<S>> seq = theSession.parseWith(theComponents.get(0));
			...
			while (true) {
				...
				while (theStack.size() < theComponents.size()) {
					...
					PossibilitySequence<? extends Expression<S>> seq = session.parseWith(theComponents.get(theStack.size()));
					Expression<S> ex = seq.getNextPossibility();
					if (ex != null) {
